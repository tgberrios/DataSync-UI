  ...
  async (req, res) => {
    try {
      ...
      if (database_name) {
        ...
        const targetConnectionString = `postgresql://${dbConfig.user}:${dbConfig.password}@${dbConfig.host}:${dbConfig.port}/${database_name}`;
...
app.get(
  ...
  async (req, res) => {
    try {
      ...
      const totalStats = result.rows.reduce(
        (acc, row) => ({
