...
const DataLineageMongoDBTreeView: React.FC<TreeViewProps> = ({ edges, onEdgeClick }) => {
  ...
  const treeData = useMemo(() => {
    ...
    edges.forEach(edge => {
      const sourceKey = `${edge.source_collection || 'Unknown'}::Collection`;
      if (!sources.has(sourceKey)) {
        sources.set(sourceKey, {
          name: edge.source_collection || 'Unknown',
          type: 'Collection',
          relationships: []
        });
      }
    ...
    return (
      <TreeNode key={edge.id}>
        ...
        >
          ...
          <NodeLabel $isTarget>{edge.target_collection || 'Unknown'}</NodeLabel>
          <RelationshipInfo>
            <Badge $type="Collection">Collection</Badge>
            {edge.target_field && (
              <Badge>Field: {edge.target_field}</Badge>
            )}
          </RelationshipInfo>
        </TreeContent>
        <ExpandableContent $isExpanded={isExpanded} $level={level}>
          {isExpanded && (
            <TargetDetails>
              ...
              {edge.database_name && (
                <div style={{ marginBottom: theme.spacing.xs }}>
                  <strong>Database:</strong> {edge.database_name}
                </div>
              )}
              {edge.collection_name && (
                <div style={{ marginBottom: theme.spacing.xs }}>
                  <strong>Collection:</strong> {edge.collection_name}
                </div>
              )}
              <div style={{ marginBottom: theme.spacing.xs }}>
...
export default DataLineageMongoDBTreeView;
